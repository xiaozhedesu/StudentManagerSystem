package club.xiaozhe.command;

import club.xiaozhe.cli.command.AddCommand;
...
class AddCommandTest {
    ...
    void testBirthdayFormatWrong() {
        ...
        InvalidValueException ex = assertThrows(InvalidValueException.class,
                () -> addCommand.execute());
        assertTrue(ex.getMessage().contains("不是一个合格的日期字符串（需要yyyy-MM-dd）或日期不合法！"));
    }
    ...
    void testEmailFormatWrong() {
        ...
        InvalidValueException ex = assertThrows(InvalidValueException.class,
                () -> addCommand.execute());
        assertTrue(ex.getMessage().contains("Email格式错误：示例：yourEmailName@example.com"));
    }
